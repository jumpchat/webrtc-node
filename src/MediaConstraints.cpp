...
MediaConstraints::MediaConstraints()
    : _audio(false)
    , _video(false)
    , _screenCastDesktopId(0)
    , _screenCastCaptureCursor(true)
    , _screenCastFPS(30)
{
...
rtc::scoped_refptr<MediaConstraints> MediaConstraints::New(const Local<Object>& constraints)
{
    ...
    if (!video_value.IsEmpty()) {
        if (video_value->IsTrue()) {
            ...
            Local<Object> video = Local<Object>::Cast(video_value);
            ...
            if (!optional_value.IsEmpty() && optional_value->IsArray()) {
                ...
                for (unsigned int index = 0; index < options->Length(); index++) {
                    ...
                    if (!option_value.IsEmpty() && option_value->IsObject()) {
                        ...
                        Local<Value> sourceId = option->Get(Nan::New("sourceId").ToLocalChecked());
                        Local<Value> screenCastDesktopId = option->Get(Nan::New("screenCastDesktopId").ToLocalChecked());
                        Local<Value> screenCastCaptureCursor = option->Get(Nan::New("screenCastCaptureCursor").ToLocalChecked());
                        Local<Value> screenCastFPS = option->Get(Nan::New("screenCastFPS").ToLocalChecked());
                        ...
                        if (!screenCastDesktopId.IsEmpty() && screenCastDesktopId->IsNumber()) {
                            self->_screenCastDesktopId = (int)screenCastDesktopId->ToInteger()->Value();
                            RTC_LOG(LS_INFO) << "**** SCREENCAST DESKTOP ID " << self->_screenCastDesktopId;
                        }

                        if (!screenCastCaptureCursor.IsEmpty() && screenCastCaptureCursor->IsBoolean()) {
                            self->_screenCastCaptureCursor = screenCastCaptureCursor->ToBoolean()->Value();
                            RTC_LOG(LS_INFO) << "**** SCREENCAST CAPTURE CURSOR " << self->_screenCastCaptureCursor;
                        }

                        if (!screenCastFPS.IsEmpty() && screenCastFPS->IsNumber()) {
                            self->_screenCastFPS = (int)screenCastFPS->ToInteger()->Value();
                            RTC_LOG(LS_INFO) << "**** SCREENCAST FPS " << self->_screenCastFPS;
                        }
                    }
                }
            }
...
void MediaConstraints::SetOptional(std::string key, Local<Value> value)
{
    ...
    if (!value.IsEmpty() && !value->IsNull() && !value->IsUndefined()) {
        if (value->IsTrue() || value->IsFalse()) {
            MediaConstraints::SetOptional(key, value->IsTrue() ? std::string(webrtc::MediaConstraintsInterface::kValueTrue) : std::string(webrtc::MediaConstraintsInterface::kValueFalse));
...
void MediaConstraints::SetMandatory(std::string key, Local<Value> value)
{
    ...
    if (!value.IsEmpty() && !value->IsNull() && !value->IsUndefined()) {
        if (value->IsTrue() || value->IsFalse()) {
            MediaConstraints::SetMandatory(key, value->IsTrue() ? std::string(webrtc::MediaConstraintsInterface::kValueTrue) : std::string(webrtc::MediaConstraintsInterface::kValueFalse));
        } else if (value->IsNumber()) {
...
int MediaConstraints::ScreenCastDesktopId() const
{
    RTC_LOG(LS_INFO) << __PRETTY_FUNCTION__;

    return _screenCastDesktopId;
}

bool MediaConstraints::ScreenCastCaptureCursor() const
{
    RTC_LOG(LS_INFO) << __PRETTY_FUNCTION__;

    return _screenCastCaptureCursor;
}

int MediaConstraints::ScreenCastFPS() const
{
    RTC_LOG(LS_INFO) << __PRETTY_FUNCTION__;

    return _screenCastFPS;
}
